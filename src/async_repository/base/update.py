...
class Update(Generic[M]): # Make class Generic
    ...
    def push(self, field: Field[Any], value: Any) -> "Update[M]":
        ...
        if self._validator:
            ...
            if not is_list:
                 raise InvalidPathError(f"Cannot $push to field '{field_path}': not a List or Optional[List].")
            # Validate the item being pushed if the list item type is known
            if item_type is not Any:
                 try:
                    self._validator.validate_value(value, item_type, f"item for $push to '{field_path}'")
                 except ValueTypeError as e:
                     # Wrap error for context
                     raise ValueTypeError(f"Invalid value for $push to '{field_path}': {e}") from e
    ...
    def pop(self, field: Field[Any], direction: int = 1) -> "Update[M]":
        ...
        if self._validator:
            ...
            if not is_list:
                 raise InvalidPathError(f"Cannot $pop from field '{field_path}': not a List or Optional[List].")
        ...
        if self._validator:
            ...
            try:
                self._validator.get_field_type(field_path)
                 ...
                 raise InvalidPathError(f"Cannot $unset field at invalid path '{field_path}': {e}") from e
    ...
    def pull(self, field: Field[Any], value_or_condition: Any) -> "Update[M]":
        ...
        if self._validator:
            is_list, item_type = self._validator.get_list_item_type(field_path)
            if not is_list:
                 raise InvalidPathError(f"Cannot $pull from field '{field_path}': not a List or Optional[List].")
            ...
            if item_type is not Any and not is_operator_dict:
                 try:
                     ...
                     self._validator.validate_value(value, item_type, f"value for $pull from '{field_path}'")
                 except ValueTypeError as e:
                     raise ValueTypeError(f"Invalid value for $pull from '{field_path}': {e}") from e
    ...
    def increment(self, field: Field[Any], amount: Union[int, float] = 1) -> "Update[M]":
    ...
    def decrement(self, field: Field[Any], amount: Union[int, float] = 1) -> "Update[M]":
        ...
        return self.increment(field, -abs(amount)) # Ensure amount is positive before negating
    ...
    def min(self, field: Field[Any], value: Union[int, float]) -> "Update[M]":
        ...
        if self._validator:
            if not self._validator.is_field_numeric(field_path):
                 raise ValueTypeError(f"Cannot apply $min to non-numeric field '{field_path}'.")
    ...
    def max(self, field: Field[Any], value: Union[int, float]) -> "Update[M]":
        ...
        if self._validator:
            if not self._validator.is_field_numeric(field_path):
                 raise ValueTypeError(f"Cannot apply $max to non-numeric field '{field_path}'.")
    ...
    def mul(self, field: Field[Any], value: Union[int, float]) -> "Update[M]":
        ...
        if self._validator:
            if not self._validator.is_field_numeric(field_path):
                 raise ValueTypeError(f"Cannot apply $mul to non-numeric field '{field_path}'.")
            # Optional: validate 'value' against the specific field type
            # self._validator.validate_value_for_path(field_path, value)
    ...
    def build(self) -> Dict[str, Any]:
        ...
        for internal_op, data in current_operations.items():
            ...
            if not mongo_op:
                self._logger.warning(f"Unknown internal op '{internal_op}' during build.")
                continue
            # Ensure data is not empty before serializing and adding
            if data:
                 # Serialize values within the data dict for the specific operator
                 serialized_data = { field_path: self._serialize_value(val)
    ...
    def __repr__(self) -> str:
        ...
        ops_repr = ", ".join(f"{op}: {data!r}" for op, data in sorted(built_ops.items()))
