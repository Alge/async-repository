...
def prepare_for_mongodb(data: Any) -> Any:
    ...
    if data is None:
        ...
        hasattr(data, "__class__") and data.__class__.__module__ == "pydantic.networks"
    ):
        return str(data)
    return data
...
async def test_store_and_get_with_url_types(repository_factory, logger):
    ...
    assert str(retrieved.metadata["stream_url"]).rstrip(
        "/"
    ) == "https://media.example.com/stream".rstrip("/")
    assert str(retrieved.metadata["config"]["websocket"]).rstrip(
        "/"
    ) == "ws://example.com:8765/".rstrip("/")
    assert str(retrieved.profile["services"]["notification_url"]).rstrip(
        "/"
    ) == "https://notify.example.com/user1".rstrip("/")
...
async def test_update_with_url_types(repository_factory, logger):
    ...
    assert str(updated.metadata["api_url"]).rstrip(
        "/"
    ) == "https://api.example.com/v1".rstrip("/")
    assert str(updated.metadata["connection"]["socket_url"]).rstrip(
        "/"
    ) == "ws://socket.example.com:9000/".rstrip("/")
...
async def test_update_nested_url_field(repository_factory, logger):
    ...
    update = Update().set("metadata.api.url", AnyHttpUrl("https://new-api.example.com"))
    ...
    assert str(updated.metadata["api"]["url"]).rstrip(
        "/"
    ) == "https://new-api.example.com".rstrip("/")
...
async def test_upsert_with_url_types(repository_factory, logger):
    ...
    assert str(retrieved.metadata["endpoint"]).rstrip(
        "/"
    ) == "https://updated.example.com/api".rstrip("/")
    assert str(retrieved.metadata["websocket"]).rstrip(
        "/"
    ) == "ws://updated.example.com:8080".rstrip("/")
...
async def test_update_many_with_url_types(repository_factory, logger):
    ...
    for entity in updated_entities:
        assert str(entity.metadata["api"]["url"]).rstrip(
            "/"
        ) == "https://common-updated.example.com/api".rstrip("/")
...
async def test_error_scenario_reproduction(repository_factory, logger):
    ...
    # Update the entity
    query_options = QueryOptions(
        expression={"and": [{"id": entity.id}, {"name": "Error Scenario Test"}]}
    )
    ...
    assert str(updated.metadata["on_outgoing_call"]["host"]).rstrip(
        "/"
    ) == "ws://alge.se:8765/".rstrip("/")
    assert updated.metadata["on_outgoing_call"]["codec"] == "pcm"
    assert updated.metadata["on_outgoing_call"]["next_action"]["type"] == "play_url"
    assert str(updated.metadata["on_outgoing_call"]["next_action"]["url"]).rstrip(
        "/"
    ) == "https://http-live.sr.se/p1-mp3-192".rstrip("/")
