...
def pytest_addoption(parser):
    parser.addoption("--run-perf-report", action="store_true", default=False,
...
def _check_postgres_linux():
    ...
    pg_version_dirs = [f"/usr/lib/postgresql/{ver}/bin" for ver in range(10, 17)]  # Check versions 10-16
    ...
    try:
        pkg_check = subprocess.run(['dpkg', '-l', 'postgresql*'],
                                   stdout=subprocess.PIPE,
                                   stderr=subprocess.PIPE,
                                   timeout=3)
...
def _check_postgres_macos():
            ...
            try:
                for version_dir in os.listdir(expanded_path):
                    ...
                    if os.path.exists(bin_path):
                        logging.info(f"Found PostgreSQL.app installation at: {bin_path}")
                        return True
    ...
    try:
        launchctl_check = subprocess.run(['launchctl', 'list', 'org.postgresql.postgres'],
                                         stdout=subprocess.PIPE,
                                         stderr=subprocess.PIPE,
                                         timeout=3)
    ...
    try:
        brew_check = subprocess.run(['brew', 'services', 'list'],
                                    stdout=subprocess.PIPE,
                                    stderr=subprocess.PIPE,
                                    timeout=3)
...
def _check_postgres_windows():
                ...
                with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, reg_path) as key:
                    ...
                    for i in range(winreg.QueryInfoKey(key)[0]):
                        ...
                        with winreg.OpenKey(key, subkey_name) as subkey:
                            try:
                                base_directory, _ = winreg.QueryValueEx(subkey, "Base Directory")
                                bin_path = os.path.join(base_directory, "bin")
                                logging.info(f"Found PostgreSQL installation in registry at: {bin_path}")
                                if os.path.exists(bin_path):
                                    return True
                            except WindowsError:
                                continue
    ...
    try:
        ...
        sc_query = subprocess.run(['sc', 'query', 'postgresql'],
                                  stdout=subprocess.PIPE,
                                  stderr=subprocess.PIPE,
                                  timeout=3)
        ...
        if sc_query.returncode != 0:
            sc_query = subprocess.run(['sc', 'query', 'postgresql-x64-14'],  # Example with specific version
                                      stdout=subprocess.PIPE,
                                      stderr=subprocess.PIPE,
                                      timeout=3)
    ...
    try:
        ...
        wsl_check = subprocess.run(['wsl', 'which', 'psql'],
                                   stdout=subprocess.PIPE,
                                   stderr=subprocess.PIPE,
                                   timeout=3)
...
def mongodb_repository_factory(mock_mongodb_conn):
    ...
    def create_repo(entity_cls, app_id_field="id", db_id_field="_id"):
        from repositories.mongodb.base import MongoDBRepository

        collection_names = {
            Entity: "entities",
        }

        collection_name = collection_names.get(entity_cls) or f"{entity_cls.__name__.lower()}s"
...
async def postgresql_repository_factory(mock_postgres_pool):
    ...
    def create_repo(entity_cls, app_id_field="id", db_id_field="_id"):
        ...
        table_name = table_names.get(entity_cls.__name__) or f"{entity_cls.__name__.lower()}s"

        print(f"Creating a postgres repo for class: {entity_cls} (entity class: {Entity}). Table name: {table_name}")
