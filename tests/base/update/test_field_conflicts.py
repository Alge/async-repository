def test_sibling_field_updates_work():
    ...
    assert_operation_present(result, SetOperation, "metadata.zipcode", {
        "value": "10001"})  # tests/base/update/test_field_conflicts.py
...
def test_duplicate_set_operations_conflict():
    """Test that multiple set operations on the same field raise ValueError."""
    update = Update(Entity)
    update.set("name", "First Name")

    with pytest.raises(ValueError) as excinfo:
        update.set("name", "Second Name")

    assert "name" in str(excinfo.value)
    assert "already has an operation" in str(excinfo.value)


def test_duplicate_push_operations_conflict():
    """Test that multiple push operations on the same field raise ValueError."""
    update = Update(Entity)
...
def test_push_pop_operations_conflict():
    """Test that push and pop operations on the same field raise ValueError."""
    update = Update(Entity)
    update.push("tags", "tag1")
...
def test_duplicate_increment_operations_conflict():
    """Test that multiple increment operations on the same field raise ValueError."""
    update = Update(Entity)
...
def test_increment_multiply_operations_conflict():
    """Test that increment and multiply operations on the same field raise ValueError."""
    update = Update(Entity)
...
def test_set_increment_operations_conflict():
    """Test that set and increment operations on the same field raise ValueError."""
    update = Update(Entity)
...
def test_unset_set_operations_conflict():
    """Test that unset and set operations on the same field raise ValueError."""
    update = Update(Entity)
...
def test_min_max_operations_conflict():
    """Test that min and max operations on the same field raise ValueError."""
    update = Update(Entity)
    update.min("value", 0)
...
def test_nested_field_conflict():
...
def test_increment_conflicts_with_decrement():
...
def test_decrement_conflicts_with_increment():
...
def test_mixed_operations_on_different_fields():
    """Test that different operation types on different fields work correctly."""
    update = Update(Entity)
...
def test_numeric_operations_on_different_fields():
    ...
    update = Update(Entity)
    update.increment("value", 5)
    update.min("float_value", 10.0)  # Use float value for float field

    result = update.build()
    assert len(result) == 2  # One operation per field
