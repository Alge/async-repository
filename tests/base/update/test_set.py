...
def test_set_with_invalid_types():
    ...
    with pytest.raises(ValueTypeError):  # Changed from TypeError
        update.set("metadata", ["not", "a", "dict"])
    ...
    with pytest.raises(
        ValueTypeError, match="missing required field 'key1'"
    ):  # Changed from TypeError
    ...
    with pytest.raises(
        ValueTypeError, match="expected type int or compatible dict, got '123'"
    ):  # Updated match
...
def test_set_with_nested_fields():
    ...
    with pytest.raises(
        ValueTypeError, match="expected type int"
    ):  # Changed from TypeError
        update.set("metadata.key2", "not-an-int")

    # Test invalid nested field path
    with pytest.raises(
        InvalidPathError, match="does not exist in type Metadata"
    ):  # Changed from TypeError
        update.set("metadata.non_existent", "value")

    # Test setting on non-existent parent
    with pytest.raises(
        InvalidPathError, match="Field 'non_existent' does not exist"
    ):  # Changed from TypeError
...
def test_set_with_complex_nested_validations():
    ...
    with pytest.raises(
        ValueTypeError, match="expected type str"
    ):  # Changed from TypeError
    ...
    with pytest.raises(
        ValueTypeError, match="missing required field 'value'"
    ):  # Changed from TypeError
