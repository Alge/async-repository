...
from tests.base.conftest import User, Address, get_field_object
...
def _create_internal_condition(
    ...
    try:
        ...
        raise AttributeError(
            f"Failed to get Field object for path '{field_path}': {e}"
        ) from e
    ...
    try:
        ...
        raise type(e)(
            f"Error applying operator '{op_name}' to Field('{field_obj.path}') with value {value!r}: {e}"
        ) from e
...
invalid_values_params = [
    ...
    (
        ...
        123,
        ValueError,
        r"Invalid filter expression: Path 'name': expected type str, got 123 \(int\)\.",
    ),
    (
        "age",
        "eq",
        "twenty",
        ValueError,
        r"Invalid filter expression: Path 'age': expected type int, got 'twenty' \(str\)\.",
    ),
    (
        "active",
        "eq",
        "true",
        ValueError,
        r"Invalid filter expression: Path 'active': expected type bool, got 'true' \(str\)\.",
    ),
    (
        "score",
        "eq",
        True,
        ValueError,
        r"Invalid filter expression: Path 'score': bool invalid for Union.*allowing int but not bool\.",
    ),
    ...
    (
        ...
        123,
        ValueError,
        r"Invalid filter expression: Operator 'contains' on field 'roles' requires value compatible with item type str, got int\. Original error: Path 'roles \(contains value\)': expected type str, got 123 \(int\)\.",
    ),
    (
        "tags",
        "contains",
        123,
        ValueError,
        r"Invalid filter expression: Operator 'contains' on field 'tags' requires value compatible with item type str, got int\. Original error: Path 'tags \(contains value\)': expected type str, got 123 \(int\)\.",
    ),
    (
        "roles",
        "in",
        ["a", 1],
        ValueError,
        r"Invalid filter expression: Invalid item in 'in' list for field 'roles'\. Path 'roles \(in item 1\)': expected type str, got 1 \(int\)\.",
    ),
    (
        "tags",
        "nin",
        {"a", 1},
        ValueError,
        r"Invalid filter expression: Invalid item in 'nin' list for field 'tags'\. Path 'tags \(nin item \d+\)': expected type str, got 1 \(int\)\.",
    ),  # Index may vary
    (
        "age",
        "in",
        [20, "30"],
        ValueError,
        r"Invalid filter expression: Invalid item in 'in' list for field 'age'\. Path 'age \(in item 1\)': expected type int, got '30' \(str\)\.",
    ),
    ...
    (
        ...
        True,
        TypeError,
        r"Operator 'startswith' requires a string value",
    ),
    ...
    (
        ...
        "maybe",
        TypeError,
        r"Operator 'exists' requires a boolean value",
    ),
    ...
    (
        ...
        "%test%",
        ValueError,
        r"Invalid filter expression: Path 'age': expected type int, got '%test%' \(str\)\.",
    ...
    (
        ...
        "adm",
        ValueError,
        r"Invalid filter expression: Path 'roles': expected type List, got 'adm' \(str\)\.",
    ),
    (
        "age",
        "gt",
        "10",
        ValueError,
        r"Invalid filter expression: Path 'age': expected type int, got '10' \(str\)\.",
    ),
    (
        "addresses[0].zipcode",
        "eq",
        "abc",
        ValueError,
        r"Invalid filter expression: Path 'addresses\.0\.zipcode': expected type int, got 'abc' \(str\)\.",
    ),  # Valid path, invalid value type
    # This case now raises ValueError directly from the filter method due to InvalidPathError
    (
        "addresses[0].nonexistent",
        "eq",
        1,
        ValueError,
        r"Invalid filter expression: Field 'nonexistent' does not exist in type Address\. Path: 'addresses\.0\.nonexistent'\. in model User",
    ),  # Invalid sub-path after index
]
invalid_values_ids = [
    f"{path.replace('.', '_').replace('[','_').replace(']','')}_{op}_{type(val).__name__}"
    for path, op, val, _, _ in invalid_values_params
]
...
invalid_path_resolution_params = [
    ...
    (
        "addresses[-1].zipcode",
        IndexError,
        r"Negative indexing is not currently supported",
    ),
    # Path accessing attribute on non-object/dict (after index)
    (
        "tags[0].subfield",
        AttributeError,
        r"Could not fully resolve field path.*part 'subfield': 'str' object has no attribute 'subfield'",
    ),
]
invalid_path_resolution_ids = [
    f"{path.replace('.', '_').replace('[','_').replace(']','')}"
    for path, _, _ in invalid_path_resolution_params
]
...
def test_single_filter_operator_expression(
    ...
    internal_condition = _create_internal_condition(
        qb_instance, field_path, op_name, value
    )
    # Build and assert
    options = qb_instance.filter(internal_condition).build()
    # Use the public path string for assertion
    public_path = field_path.replace("[", ".").replace(
        "]", ""
    )  # Convert test path to validator format
    assert_expression_present(
        options.expression,
        QueryFilter,
        field_path=public_path,
        operator=expected_op_enum,
        expected_value=expected_value_in_filter,
    )
...
def test_multiple_sequential_filters_expression(qb: QueryBuilder[User]):
    ...
    assert options.expression.operator == "and"
    assert (
        len(options.expression.conditions) == 2
    )  # Still 2 direct conditions before translation flattening
...
def test_random_order_option(qb: QueryBuilder[User]):
    ...
    assert (
        options.random_order is True
        and options.sort_by is None
        and options.sort_desc is False
    )


def test_combined_query_options(qb: QueryBuilder[User]):
    options = (
        qb.filter(qb.fields.active == True)
        .sort_by(qb.fields.score, descending=True)
        .limit(25)
        .offset(5)
        .timeout(10.0)
        .build()
    ...
    assert (
        options.limit == 25
        and options.offset == 5
        and options.timeout == 10.0
        and options.random_order is False
    )
...
def test_valid_field_access_returns_field_object(qb: QueryBuilder[User]):
    ...
    assert (
        isinstance(qb.fields.addresses, Field)
        and qb.fields.addresses.path == "addresses"
    )
    # Nested attributes
    assert (
        isinstance(qb.fields.metadata.timestamp, Field)
        and qb.fields.metadata.timestamp.path == "metadata.timestamp"
    )
    assert (
        isinstance(qb.fields.addresses[0], Field)
        and qb.fields.addresses[0].path == "addresses.0"
    )
    assert (
        isinstance(qb.fields.addresses[10].street, Field)
        and qb.fields.addresses[10].street.path == "addresses.10.street"
    )
...
def test_invalid_field_access_raises_attribute_error(qb: QueryBuilder[User]):
    ...
    with pytest.raises(
        AttributeError, match=r"object has no attribute 'nonexistent_field'"
    ):
...
def test_invalid_path_resolution_raises_error(
...
def test_filter_with_valid_values_succeeds(qb: QueryBuilder[User]):
    ...
    try:
        ...
        QueryBuilder(model).filter(
            fields.addresses[1].street == "Street street 123"
        ).build()
        QueryBuilder(model).filter(fields.addresses[0].city != "Unknown").build()
        QueryBuilder(model).filter(
            fields.addresses == [Address(), Address(), Address()]
        ).build()
...
def test_filter_with_invalid_values_or_path_raises_error(  # Renamed for clarity
    ...
    qb_instance = QueryBuilder(user_model)
    if field_path == "is_active":
        field_path = "active"  # Legacy correction
...
def test_repr_methods(qb: QueryBuilder[User]):
    ...
    assert (
        "expression=QueryFilter(field_path='addresses.0.city', operator=<QueryOperator.EQ: 'eq'>, value='LA')"
        in options_repr
    )
...
def test_model_less_builder():
    ...
    assert qb_generic._validator is None and isinstance(
        qb_generic.fields, GenericFieldsProxy
    )
    ...
    assert (
        isinstance(deep_field, Field) and deep_field.path == "level1.level2.final_name"
    )

    # --- NEW: Indexed access without model ---
    indexed_field = qb_generic.fields.some_list[5].value
    assert (
        isinstance(indexed_field, Field) and indexed_field.path == "some_list.5.value"
    )

    # --- NEW: String key access (should now work) ---
    dict_field = qb_generic.fields.bar["key"]
    assert isinstance(dict_field, Field) and dict_field.path == "bar.key"
    ...
    try:
        ...
        assert_expression_present(
            options.expression,
            QueryFilter,
            field_path="data.payload.status",
        ...
        assert (
            options.sort_by == "num_field"
            and options.sort_desc is True
            and options.limit == 10
        )
    except (ValueError, TypeError, AttributeError) as e:
        pytest.fail(
            f"Model-less builder failed unexpectedly: {e}\n{traceback.format_exc()}"
        )
    ...
    # IndexError from Field.__getitem__ still works for negative index
    with pytest.raises(IndexError, match="Negative indexing"):
        _ = qb_generic.fields.bar[-1]

    # Check other invalid types for __getitem__ still raise TypeError
    with pytest.raises(TypeError, match="Field index must be an integer or string"):
        _ = qb_generic.fields.bar[1.5]  # e.g., float
    with pytest.raises(TypeError, match="Field index must be an integer or string"):
        _ = qb_generic.fields.bar[None]  # e.g., None
