...
async def test_dsl_operator_overloading_query(repository_factory, logger):
    ...
    expr = (((qb.value > 21) & (qb.active == True)) | (qb.name.like("%Alice%"))) & (qb.owner == "org1")
    ...
    assert count == len(listed), f"Count {count} does not match listed length {len(listed)}"
...
async def test_dsl_filter_equal(repository_factory, logger):
    ...
    for ent in [Entity(name="Alice"), Entity(name="Bob"), Entity(name="Alice")]:
        await repo.store(ent, logger)
    options = QueryOptions(expression={"name": {"operator": "eq", "value": "Alice"}}, limit=100, offset=0)
    listed = []
    async for item in repo.list(logger, options):
        listed.append(item)
...
async def test_dsl_complex_filter(repository_factory, logger):
    ...
    """
    repo = repository_factory(Entity)
    for ent in [Entity(name="Alice", value=50), Entity(name="Bob", value=150), Entity(name="Charlie", value=250)]:
        await repo.store(ent, logger)
    expr = {
        "or": [
            {"and": [
...
async def test_dsl_string_filters(repository_factory, logger):
    ...
    for ent in [Entity(name="Alpha"), Entity(name="Beta"), Entity(name="Gamma")]:
        await repo.store(ent, logger)
    # Test 'startswith'
    options = QueryOptions(expression={"name": {"operator": "startswith", "value": "Al"}}, limit=100, offset=0)
    listed = []
    async for item in repo.list(logger, options):
        listed.append(item)
    assert len(listed) == 1
    ...
    # Test 'endswith'
    options = QueryOptions(expression={"name": {"operator": "endswith", "value": "ta"}}, limit=100, offset=0)
    listed = []
    async for item in repo.list(logger, options):
        listed.append(item)
    assert len(listed) == 1
    ...
    options = QueryOptions(expression={"name": {"operator": "like", "value": "am"}}, limit=100, offset=0)
    listed = []
    async for item in repo.list(logger, options):
        listed.append(item)
    assert len(listed) == 1
...
async def test_dsl_array_contains(repository_factory, logger):
    ...
    options = QueryOptions(expression={"tags": {"operator": "contains", "value": "admin"}}, limit=100, offset=0)
    listed = []
    async for item in repo.list(logger, options):
        listed.append(item)
    assert len(listed) == 1
    ...
    # Test another value
    options = QueryOptions(expression={"tags": {"operator": "contains", "value": "user"}}, limit=100, offset=0)
    listed = []
    async for item in repo.list(logger, options):
        listed.append(item)
    names = {item.name for item in listed}
...
async def test_dsl_sorting(repository_factory, logger):
    ...
    repo = repository_factory(Entity)
    for ent in [Entity(name="A", value=300), Entity(name="B", value=100), Entity(name="C", value=200)]:
        await repo.store(ent, logger)
    options = QueryOptions(expression={}, sort_by="value", sort_desc=False, limit=100, offset=0)
    listed = []
    async for item in repo.list(logger, options):
        listed.append(item)
...
async def test_dsl_operator_gt(repository_factory, logger):
    ...
    for ent in [Entity(value=50), Entity(value=150), Entity(value=250)]:
        await repo.store(ent, logger)
    options = QueryOptions(expression={"value": {"operator": "gt", "value": 100}}, limit=100, offset=0)
    listed = []
    async for item in repo.list(logger, options):
        listed.append(item)
...
async def test_dsl_operator_in(repository_factory, logger):
    ...
    for ent in [Entity(name="Alice"), Entity(name="Bob"), Entity(name="Charlie")]:
        await repo.store(ent, logger)
    options = QueryOptions(expression={"name": {"operator": "in", "value": ["Alice", "Charlie"]}}, limit=100, offset=0)
    listed = []
    async for item in repo.list(logger, options):
        listed.append(item)
    names = {item.name for item in listed}
...
async def test_dsl_operator_nin(repository_factory, logger):
    ...
    for ent in [Entity(name="Alice"), Entity(name="Bob"), Entity(name="Charlie")]:
        await repo.store(ent, logger)
    options = QueryOptions(expression={"name": {"operator": "nin", "value": ["Bob"]}}, limit=100, offset=0)
    listed = []
    async for item in repo.list(logger, options):
        listed.append(item)
    names = {item.name for item in listed}
...
async def test_dsl_operator_exists(repository_factory, logger):
    ...
    for ent in [Entity(owner="org1"), Entity(owner=None), Entity(owner="org2")]:
        await repo.store(ent, logger)
    options = QueryOptions(expression={"owner": {"operator": "exists", "value": True}}, limit=100, offset=0)
    listed = []
    async for item in repo.list(logger, options):
        listed.append(item)
...
async def test_dsl_operator_regex(repository_factory, logger):
    ...
    for ent in [Entity(name="Alice"), Entity(name="Alicia"), Entity(name="Bob")]:
        await repo.store(ent, logger)
    options = QueryOptions(expression={"name": {"operator": "regex", "value": "^Ali"}}, limit=100, offset=0)
    listed = []
    async for item in repo.list(logger, options):
        listed.append(item)
    names = {item.name for item in listed}
